 Р.form 'ctrlbuh_106'
.nameinlist 'Отчет о наличии записей в книге покупок'
...
.endvar
.create view t1
AS SELECT oborot.*,schfact.*,bookprzk.*,katorg.*
WHERE
((
 '768'==oborot.scheto
 AND '04'==oborot.subossch
 AND d1<<=oborot.datob
 AND d2>>=oborot.datob
 ...
 AND oborot.schetk='719'
 AND oborot.kauos[1]=00010000000018B5h
 AND NOT IsValidAll(tnbookprzk);
.begin
 npp:=0;
 d1:=DGetTune('Dat_m_ot');
 d2:=DATE(LAST_DAY(d1),MONTH(d1),YEAR(d1));
 IF RunDialog('C_common::GetIntervalDates',d1,d2)<>cmOK ctrlbuh_106.fexit;
end.
.fields
 d1
 d2
